...
const isLikelyHeaderLine = (line) => {
  ...
  const hasHeaderWords =
    /\b(date|txn date|value date|narration|description|particulars|withdrawal|deposit|debit|credit|balance|amount|statement)\b/.test(
      normalized,
    );
...
const parseIndianBankStatementLines = (lines) => {
  ...
  while (index < lines.length) {
    ...
    if (!line || PDF_SKIP_LINE_REGEX.test(line)) {
      index += 1;
      continue;
    }
...
const parsePdfLines = (positionLines, fallbackLines = positionLines) => {
  const indianBankTransactions = parseIndianBankStatementLines(positionLines);
  if (indianBankTransactions.length > 0) {
    return indianBankTransactions;
  }

  return parseSingleLinePdfLines(fallbackLines);
};
...
const parsePdfViaApi = async (file) => {
  ...
  return { transactions: payload.transactions, errors: [] };
};
